...
class NyhederPipeline:

    # Database setup
    conn = sqlite3.connect('nyheder.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `nyheder`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM nyheder WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO nyheder (date, title, link, content)"
